...
import re
import glob
db_type='uniprot' #uniprot or metagenome-assembled
...
def writeout(proteindict, psmdict):
    with open(folder+'/psm.tsv','w') as f:
        ...
        for key in psmdict:
            ...
            #string.append(psmdict[key].proteinDesc[:40])
            string.append('0')
            #string.append(','.join(list(set(psmdict[key].mappedGenes))[:2]))
            #string.append(','.join(list(set(psmdict[key].mappedProteins))[:2]))
            string.append('0')
            string.append('0')
            f.write('\t'.join(string)+'\n')
    ...
    with open(folder+'/protein.tsv','w') as f:
        ...
        for key in proteindict:
            ...
            string.append('0')
            string.append('0')
            #string.append(','.join(list(set(proteindict[key].modifications))[:2]))
            string.append('0')
            string.append('')
            #string.append(','.join(list(set(proteindict[key].indistinguishable_proteins))[:2]))
            string.append('0')
            f.write('\t'.join(string)+'\n')
