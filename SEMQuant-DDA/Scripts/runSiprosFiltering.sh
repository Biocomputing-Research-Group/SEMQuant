...
# Generate PSM table
if [ "$TabFile" == "" ]; then
TabFile=$(python3 ${exePath}/sipros_psm_tabulating.py -i ${SipFolder}/ -c ${ConfigureFile} -o ${OutputFolder}/)
fi

# PSM Filtering
# echo ${TabFile}
python3 ${exePath}/sipros_ensemble_filtering.py -i ${TabFile} -c ${ConfigureFile} -o ${OutputFolder}/

# Protein Assembly
python3 ${exePath}/sipros_peptides_assembling.py -w ${OutputFolder}/ -c ${ConfigureFile}
